...
export default function PublicationForm({ kind }: { kind: "news" | "tender" }) {
  ...
  return <form onSubmit={submit} style={{ display: "grid", gap: 12, border: "1px solid #d5d9df", borderRadius: 8, padding: 18 }}>
    ...
    <label>Treść
      <div style={{ display: "flex", gap: 6, margin: "6px 0" }}><button type="button" onClick={() => format("b")}><strong>B</strong></button><button type="button" onClick={() => format("i")}><em>I</em></button><button type="button" onClick={() => format("h2")}>Nagłówek</button></div>
    ...
    <label className="button button-outline" style={{ width: "fit-content", cursor: "pointer" }}>Dodaj grafikę<input name="image" type="file" accept="image/jpeg,image/png,image/webp" style={{ display: "none" }} /></label>
    ...
    <label className="button button-outline" style={{ width: "fit-content", cursor: "pointer" }}>Dodaj pliki<input name="attachments" type="file" accept=".pdf,.docx,.xlsx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" multiple style={{ display: "none" }} /></label>
