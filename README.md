...
## Publikacja repozytorium na GitHub

Ponizej najszybsza sciezka, aby wrzucic projekt do nowego repo:

1. Utworz puste repozytorium na GitHub (bez README, .gitignore i License).
...
Po pushu workflow deployu z [deploy-cloudflare.yml](.github/workflows/deploy-cloudflare.yml) uruchomi sie automatycznie na branchu `main`.

W GitHub ustaw jeszcze:

- Secrets: `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`
- Variable: `NEXT_PUBLIC_SITE_URL`

Jesli artefakty lokalne byly kiedys dodane do gita, przed pierwszym pushem wykonaj porzadkowanie:

```bash
git rm -r --cached dist node_modules .wrangler .openai .vscode package-lock.json
git commit -m "Cleanup tracked local/build artifacts"
```
